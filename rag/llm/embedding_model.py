...
class OpenAI_APIEmbed(OpenAIEmbed):
    ...
    _OCI_ADAPTER_HOST = "host.docker.internal"
    _OCI_ADAPTER_PORT = 8181
    _OCI_EMBED_DIMENSIONS = 1536
    ...
    def __init__(self, key, model_name, base_url):
        ...
        self._uses_oci_embed_v4 = (
            self.model_name == self._OCI_EMBED_MODEL
            and parsed_url.hostname == self._OCI_ADAPTER_HOST
            and parsed_url.port == self._OCI_ADAPTER_PORT
        )

    def _oci_embed_call(self, batch, input_type):
        """Embed through OCI with an explicit Cohere Embed 4 role."""
        res = self.client.embeddings.create(
            input=batch,
            model=self.model_name,
            dimensions=self._OCI_EMBED_DIMENSIONS,
            encoding_format="float",
    ...
    def encode(self, texts: list):
        if not self._uses_oci_embed_v4:
            return super().encode(texts)
        return self._batched_encode(
            texts,
            lambda batch: self._oci_embed_call(batch, "SEARCH_DOCUMENT"),
            batch_size=16,
            truncate_to=8191,
        )
    ...
    def encode_queries(self, text):
        ...
        vectors, token_count = self._batched_encode(
            [text],
            lambda batch: self._oci_embed_call(batch, "SEARCH_QUERY"),
            batch_size=16,
            truncate_to=8191,
        )
        return vectors[0], token_count
